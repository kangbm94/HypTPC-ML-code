...
Tag= "Xi_"+str(runnum)+"_"+str(frag+1)+"/1000"
...
outfilename = "Templ0"+str(runnum)+"_"+str(frag)+".root"
...
print(layer_names)
for layer_name, layer in zip(layer_names,model.layers)
   if 'conv2d' not in layer_name:
       continue
   n_feature = layer.shape[-1]    
   size = layer.shape[1]    
   ncol = n_features

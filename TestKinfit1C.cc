...
//		V1.SetPhi(PhLdMeas);
		...
		ThLdMeas = LdReconMeas.Theta();
		PhLdMeas = LdReconMeas.Phi();
...
//		double Variance[8] = {ResThV*ResThV,ResPhV*ResPhV,rp*rp,ResThP*ResThP,ResPhP*ResPhP,rpi1*rpi1,ResTh*ResTh,ResPh*ResPh};
		...
		double Variance[8] = {rp*rp,ResThP*ResThP,ResPhP*ResPhP,rpi1*rpi1,ResTh*ResTh,ResPh*ResPh,0,0};
...
//		auto LdCor = cont.at(2);
		...
		auto sigp = sqrt(abs(UV(0,0)));
		auto sigth = sqrt(abs(UV(1,1)));
		auto sigph = sqrt(abs(UV(2,2)));
		auto dPLd = (PLd - PLdCor)/sigp;
		auto dThLd = (ThLd - ThLdCor)/sigth;	
		auto dPhLd = (PhLd - PhLdCor)/sigph;	
//		auto dPLd = (PLd - PLdCor);
//		auto dThLd = (ThLd - ThLdCor);
//		auto dPhLd = (PhLd - PhLdCor);	
//		UV.Print();
		tempPull.push_back(dPLd);
		tempPull.push_back(dThLd);
		tempPull.push_back(dPhLd);
		for(auto p:UPullLd){

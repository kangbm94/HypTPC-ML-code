...
void FADC(){
	...
	TString dir = "../../MayRun/rootfiles/FADC/";
	TString filename = Form("run0%d_TPCWaveform.root",runnum);
	//	TString filename = Form("run0%d_TPC_RMSCut.root",runnum);
...
void FADC(int dum){
	...
	TCanvas* c1 = new TCanvas("c2","c2",1000,1000);
	//	c1->Divide(2,1);
...
//	pad2->Divide(2,1);
	...
	TLine* l1 = new TLine(1,1,2,2);
	TLine* l2 = new TLine(1,1,2,2);
	TLine* l3 = new TLine(1,1,2,2);
	TLine* l4 = new TLine(1,1,2,2);
	l1->SetLineColor(kBlue);
	l2->SetLineColor(kRed);
	l3->SetLineColor(kBlack);
	l4->SetLineColor(kRed);
	c1->cd(1);
	leg->AddEntry(l1,"Corrected Waveform","l");
	leg->AddEntry(l2,"Raw Waveform","l");
	leg->AddEntry(l3,"baseline","l");
//	leg->AddEntry(l4,"orrected Waveform","1");
	...
	for(int i=start;i<start+ent;++i){
		...
		//base->Draw();
		auto layers = *FMan.GetLayers();
		auto rows = *FMan.GetRows();
//		vector<int> layers= {0};
//		vector<int> rows= {25};
		for(int j=0;j<1;++j){
			cout<<"W : "<<j<<endl;
			wh[j] = FMan.GetWaveformHist(layers[j],rows[j]);
			wh[j]->GetXaxis()->SetTitle("time [ns]");
			wh[j]->GetYaxis()->SetTitle("amplitude [arb]");
			rh[j] = FMan.GetRawWaveformHist(layers[j],rows[j]);
//			if(cnt>1)continue;
			wh[j]->GetXaxis()->SetNdivisions(17,false);
			wh[j]->GetYaxis()->SetNdivisions(10,false);
			if(wh[j]!=nullptr){
				cnt++;
			//	pad2->cd(cnt);
				pad2->cd();
				wh[j]->Draw();
				wh[j]->SetLineColor(kBlue);
				wh[j]->SetLineWidth(3);
				rh[j]->SetLineColor(kRed);
				rh[j]->SetLineWidth(3);
				wh[j]->GetYaxis()->SetRangeUser(-100,700);
				rh[j]->Draw("same");
				rh[j]->SetTitle(Form("LR=(%d,%d),ev=%d",layers[j],rows[j],i));
				base->Draw("same");
				base->SetLineColor(kBlack);
				base->SetLineWidth(3);
				leg->Draw("same");
				int npeaks = FMan.DoFit(layers[j],rows[j],par);	
			}
		}
		/*
			 for(int j=cnt;j<1;++j)	

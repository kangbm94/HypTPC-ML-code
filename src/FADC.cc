...
vector<int> flayers;//={31,31,31,31,31,31,31,31,31,31};
vector<int> frows;//={60,61,62,63,64,65,66,67,68,69};
int n_fadc = fpads.size();
void FADC(){
	for(int i=0;i<n_fadc;++i){
//		flayers.push_back(tpc::getLayerID(fpads[i]));
//		frows.push_back(tpc::getRowID(fpads[i]));
	}
...
//	int ent2 = gTPCManager.GetEntries();
	...
	TString filename = Form("run0%d_TPCWaveform_1K.root",runnum);
	cout<<"FMAN"<<endl;
	FADCManager FMan;
	cout<<"FMANFile"<<endl;

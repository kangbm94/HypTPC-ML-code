...
*/		
		...
		Chi2=		KF.DoKinematicFit();
		NStep = KF.GetNStep();
		BestStep = KF.GetBestStep();
		auto stepChi2 = KF.GetStepChi2();
		auto stepMassDiff = KF.GetStepMassDiff();
		for(int i=0;i<200;++i){
			StepChi2[i]=0;
			StepMassDiff[i]=0;
			Step[i]=-1;
...
//		auto XiFit = LdFit + Pi2Meas;
		...
		double prob = ROOT::Math::chisquared_cdf(Chi2,KF.GetNDF());
		HistProb->Fill(prob);
